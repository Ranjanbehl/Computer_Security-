...
for n in range(0, bv4.length(), 1024):
    ...
    for i in range(16, 80):
        sigma0 = (words[i-15].deep_copy() >> 1) ^ (words[i-15].deep_copy() >> 8) ^ \
                 (words[i-15].deep_copy().shift_right(7))

        sigma1 = (words[i-2].deep_copy() >> 19) ^ (words[i-2].deep_copy() >> 61) ^ \
                 (words[i-2].deep_copy().shift_right(6))

        temp = (int(words[i-16]) + int(sigma0) + int(words[i-7]) + int(sigma1)) & mod64
        words[i] = BitVector(intVal=temp, size=64)

        a, b, c, d, e, f, g, h = h0, h1, h2, h3, h4, h5, h6, h7

    for i in range(80):
        T1 =    (int(h) + int(((e & f) ^ (~e & g)))
              + int(((e.deep_copy() >> 14) ^ (e.deep_copy() >> 18) ^ (e.deep_copy() >> 41)))
              + int(words[i]) + int(BitVector(hexstring=k[i]))) & mod64

        T2 =    (int(((a.deep_copy() >> 28) ^ (a.deep_copy() >> 34) ^ (a.deep_copy() >> 39)))
              + int(((a & b) ^ (a & c) ^ (b & c)))) & mod64

        h = g
        g = f
        f = e
        e = BitVector(intVal=((int(d) + T1) & mod64), size=64)
        d = c
        c = b
        b = a
        a = BitVector(intVal=((T1 + T2) & mod64), size=64)

    a_new = (int(a) + int(h0)) & mod64
    b_new = (int(b) + int(h1)) & mod64
    c_new = (int(c) + int(h2)) & mod64
    d_new = (int(d) + int(h3)) & mod64
    e_new = (int(e) + int(h4)) & mod64
    f_new = (int(f) + int(h5)) & mod64
    g_new = (int(g) + int(h6)) & mod64
    h_new = (int(h) + int(h7)) & mod64

    h0 = BitVector(intVal=a_new, size=64)
    print(h0)
    h1 = BitVector(intVal=b_new, size=64)
    h2 = BitVector(intVal=c_new, size=64)
    h3 = BitVector(intVal=d_new, size=64)
    h4 = BitVector(intVal=e_new, size=64)
    h5 = BitVector(intVal=f_new, size=64)
    h6 = BitVector(intVal=g_new, size=64)
    h7 = BitVector(intVal=h_new, size=64)


hash = h0 + h1 + h2 + h3 + h4 + h5 + h6 + h7

with open("output.txt", 'w') as fp:
    fp.write(hash.get_bitvector_in_hex())


if __name__ == '__main__':
    import hashlib
    print (hash.get_bitvector_in_hex())
    print (hashlib.sha512(message.encode()).hexdigest())