...
def KeyExpansion(keyBv,numRounds):
   #create the key expansion array
    ...
    for i in range(num_rounds+1):
        ...
        #roundKeys.append(key_words[i*4] + key_words[i*4+1] + key_words[i*4+2] + key_words[i*4+3]) # this makes it slower somehow
        roundKeys.append(round_keys[i])
    #return round_keys

def genRoundKeys(keyStr):
   #open key file
    keyfp = open(keyStr,'r')
    keyText = keyfp.read()
    key = keyText.strip()
    ...
    #close key file
    keyfp.close()
    KeyExpansion(key_bv,14) 

def encrypt(plainText,keyStr):
    bv = plainText #plaintext is the vector based in for x931
    ...
    bvRead = bvRead ^ BitVector(bitstring = roundKeys[0])
...
#convert pt block to input state array
    ...
    for i in range(14):
        ...
        bvRead ^= roundKeys[i + 1]
        stateArray = bvToSa(bvRead)
        #return result as a bv
    cipherText = bvRead
    return cipherText
